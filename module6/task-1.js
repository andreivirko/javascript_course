...
//   return users.sort((a, b) => a.friends.length - b.friends.length).map((user) => username);
// };
...
//   return users.reduce((acc, user) => {acc.push(...user.skills); return acc;}, []).filter((user, index, array) => array.indexOf(user) === index).sort();
// };
